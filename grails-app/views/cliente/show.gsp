	...
	<body>
		...
		<div id="show-cliente" class="content scaffold-show" role="main">
			...
			<ol class="property-list cliente">
				...
				<g:if test="${clienteInstance?.produtosPreferidos}">
				<li class="fieldcontain">
					<span id="produtosPreferidos-label" class="property-label"><g:message code="cliente.produtosPreferidos.label" default="Produtos Preferidos" /></span>
					
						<g:each in="${clienteInstance.produtosPreferidos}" var="p">
						<span class="property-value" aria-labelledby="produtosPreferidos-label"><g:link controller="produto" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
