		...
		<div id="list-itemPedido" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="quantidadePedido" title="${message(code: 'itemPedido.quantidadePedido.label', default: 'Quantidade Pedido')}" />
					
						<g:sortableColumn property="valorDaVenda" title="${message(code: 'itemPedido.valorDaVenda.label', default: 'Valor Da Venda')}" />
				...
				<g:each in="${itemPedidoInstanceList}" status="i" var="itemPedidoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${itemPedidoInstance.id}">${fieldValue(bean: itemPedidoInstance, field: "quantidadePedido")}</g:link></td>
					
						<td>${fieldValue(bean: itemPedidoInstance, field: "valorDaVenda")}</td>
